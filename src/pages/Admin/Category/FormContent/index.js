...
const FormContent = ({values, setFieldValue, errors, touched, history, isSubmitting, isUpdate = false, t}) => {
    ...
    const imageUpload = () => {
        let numFor = values.photo.length
        let iFor = 0
        for (let i = 0; i < values.photo.length; i++) {
            let imageFile =  values.photo[i].file;
            let nameFile = values.photo[i].file.name
            iFor = 1 + i
            function encode(name) {
                return window.btoa(name);
            }

            let inputFileName = values.photo[i].file.name,
            ...
            let uploadTask = storageFirebase.ref(`category/${filenames}`).put(imageFile);
            if (iFor === numFor) {
            uploadTask.on(
                "state_changed",
                (snapshot) => {
