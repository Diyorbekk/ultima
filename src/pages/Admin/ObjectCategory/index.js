...
const CompanyType = () => {
  ...
  const deleteObject = () => {
    ...
    dispatch(Actions.DELETE.request({
      method: "POST",
      url: '/admin/oc/object-category/'+objectId,
      name: 'objectCategory',
      id: objectId,
      cb: {
        success: () => {
  ...
  return (
    <>
      ...
      >
        <div className={"p-4"}>
          <div className={"mb-5 fw-medium font-size-18"}>
            Категория сложности объекта: {get(selectedObject,'name')}
          </div>
          <div className={"text-end"}>
            <button className={"btn btn-primary text-white focus-none"} onClick={() => setObject(null)}>Закрыть</button>
          </div>
        </div>
      </Modal>
      <Modal
          ...
          <MyForm
            ...
            onSubmit={({ values, setSubmitting }) => {
              ...
              dispatch(Actions.METHOD.request({
                url: '/admin/oc/object-category/'+updateObject.id,
                id: updateObject.id,
                name: 'objectCategory',
                values: serialize(values),
                cb: {
      ...
      <div className="d-flex align-items-center justify-content-between py-4">
        <h1 className="font-size-24 text-dark-blue fw-semibold">Категория сложности объекта</h1>
              ...
              isFetched && data.length
                ? <div className="table-responsive">
                  <table className="table table-borderless table-separate spaceY-1 rounded-bottom">
                    ...
                    <tr>
                      <th className="px-lg-4 p-3 fw-normal" style={{ width: 50}}>№</th>
                      <th className="px-lg-4 p-3 fw-normal">Категория сложности объекта</th>
                    ...
                    <tbody className="row-bg-white">
                    {
                      data.length
                        ? data.map((item, key) => (
                          <tr key={key}>
                            <td className="fw-medium p-3 p-lg-4">
                              <span className={"pr-3"}>{key + 1}</span>
                            </td>
                            <td className="fw-medium p-3 p-lg-4">{get(item,'name')}</td>
                            <td className="fw-medium p-3 p-lg-4 align-middle" style={{ width: 200}}>
                              <div className={"d-flex align-items-center justify-content-around"}>
                                <span
                                  onClick={() => setUpdate(item)}
                                  className={"cursor-pointer text-secondary mx-2 text-primary-hover"}
                                >
                  ...
                  <table className="table table-borderless table-separate spaceY-1 rounded-bottom">
                    <thead className="bg-primary text-white">
                      <tr>
                        ...
                        <th className="p-3 fw-normal">Категория сложности объекта</th>
                    ...
                    <tbody className="row-bg-white">
                      {
                        [...Array(10).keys()].map((item, key) => (
                          <tr key={key}>
                            <td className="p-3" style={{ width: 50}}><div className={"skeleton rounded-circle"} style={{ width: 25, height: 25}}/></td>
