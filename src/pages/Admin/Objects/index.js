...
const CompanyType = () => {
  ...
  const deleteObject = () => {
    ...
    dispatch(Actions.DELETE.request({
      url: '/admin/oc/object/'+selectedObject,
      id: selectedObject,
      name: 'objectList',
      cb: {
        success: () => {
    ...
    <>
      ...
      >
        <div className={"p-4"}>
          ...
          <div className={"text-center"}>
            <button className={"ml-3 btn btn-primary text-white btn-sm focus-none"} onClick={() => setSelectedObject(null)}>Отмена</button>
            <button className={"ml-3 btn btn-danger btn-sm focus-none"} onClick={deleteObject}>Удалить</button>
          </div>
        </div>
      </Modal>
      <div className="d-flex align-items-center justify-content-between py-4">
        ...
        <div>
          ...
          <Link
            className="btn btn-primary text-white focus-none ml-3"
            to="/admin/construction-control/objects/create"
          >Добавить</Link>
        </div>
      </div>
      ...
      >
        {({ isFetched, data=[], meta={}}) => {
          ...
          return <>
            <Spin
              isSpinning={(!isFetched && data.length) || isLoading}
            >
              {
                data.length
                ? <div className={"info-group"}>
                    {
                      data.map((item,key) => (
                        <div key={key} className={`info-item mb-2 ${get(item,'my_gov') ? 'info-item-green' : 'bg-white'} p-3 rounded-3 ${isObject === item.id ? 'active' : ''}`}>
                          ...
                          <div className={`d-flex align-items-center ${isObject === item.id ? 'justify-content-between' : 'justify-content-end'}`}>
                            <div className={`d-flex align-items-center ${isObject !== item.id ? 'visually-hidden' : ''}`}>
                              <Link
                                className={"mr-3 p-1"}
                                to={`#!`}
                                // to={`/admin/construction-control/objects/${item.id}/edit`}
                              >
                                <i className={"fas fa-pen"}/>
                              </Link>
                              <Link className={"mr-3 p-1"} to={`/admin/construction-control/objects/${item.id}/view`}><i className={"fas fa-eye"}/></Link>
                              <span className={"p-1 text-primary cursor-pointer"} onClick={() => setSelectedObject(item.id)}><i className={"fas fa-trash"}/></span>
                            </div>
