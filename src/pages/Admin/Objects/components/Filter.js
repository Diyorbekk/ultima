...
const Filter = ({ onChange, onClear, value }) => {
  const search = qs.parse((useLocation()).search);
  const [state, setState] = useState({
    id: get(search,'id', ''),
    filter_by_date: get(search, 'filter_by_date', (new Date()).getFullYear()),
    name_building: get(search, 'name_building',''),
    company: get(search, 'company',''),
    inspector: get(search, 'inspector',''),
    region_id: get(search, 'region_id',''),
    district_id: get(search, 'district_id',''),
    registration_number: get(search, 'registration_number','')
  });
